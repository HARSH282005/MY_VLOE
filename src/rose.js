...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function startCardLoader() {
        const duration = 2400;
        const steps = [
            { threshold: 20, text: 'Loading for love...' },
    ...
    function createSepals() {
        ...
        for (let i = 0; i < SEPALS_COUNT; i++) {
            ...
            if(calyx) calyx.appendChild(sepal);
        }
    }
    ...
    function createPetals() {
        PETAL_LAYERS.forEach((layer, li) => {
            ...
            for (let i = 0; i < layer.count; i++) {
                ...
                if(roseHead) roseHead.appendChild(petal);
            }
        });
    }
    ...
    function growStem() {
        return new Promise(resolve => {
            if(stem) stem.classList.add('grow');

            setTimeout(() => {
                if(leafLeft) leafLeft.classList.add('visible');
            }, 800);

            setTimeout(() => {
                if(leafRight) leafRight.classList.add('visible');
            }, 1100);
    ...
    function bloom() {
        if(calyx) calyx.classList.add('visible');
        if(ambientLight) ambientLight.classList.add('visible');
        if(roseHead) roseHead.classList.add('blooming');
    }

    function spawnFallingPetal() {
        if (!fallingPetalsEl || fallingPetalsEl.childElementCount > 40) return;

        const petal = document.createElement('div');
        petal.className = 'falling-petal';
        
        // Use the whole word 'love'
        petal.innerText = 'love';
    ...
    async function startAnimationSequence() {
        ...
        setTimeout(() => {
            if(roseWrapper) roseWrapper.classList.add('rotating');
        }, 2600);
    ...
    // Instead of starting the loader immediately on DOM load (when it's hidden),
    // wait until the blank slide is shown.
    // We can hook into a custom event or a global function we expose.
    window.startRoseLoader = function() {
        setTimeout(() => {
            startCardLoader();
        }, 400);
    };
